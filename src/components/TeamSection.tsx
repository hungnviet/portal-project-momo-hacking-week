  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
      ...
      {notDoneTasks.length > 0 && (
        <div className="border-t border-gray-200 px-4 py-2 bg-indigo-100 border-indigo-200">
        ...
        <div className="border-t border-gray-200 bg-gray-50">
          <div className="max-h-96 overflow-y-auto">
            {notDoneTasks.map((task, index) => (
              <div
                key={task.taskId}
                className={`p-3 border-b border-gray-200 last:border-b-0 ${isOverdue(task.duedate) ? 'bg-red-50 border-red-200' : ''
                  }`}
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="flex-1 min-w-0">
                    <h4 className={`text-sm font-medium truncate ${isOverdue(task.duedate) ? 'text-red-900' : 'text-gray-900'
                      }`}>
                  ...
                  <div className="flex flex-col items-end ml-2">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${task.status.toLowerCase().includes('progress')
                    ...
                    {task.duedate && (
                      <div className={`text-xs mt-1 flex items-center ${isOverdue(task.duedate) ? 'text-red-600 font-medium' : 'text-gray-500'
                        }`}>
                        <svg className="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
